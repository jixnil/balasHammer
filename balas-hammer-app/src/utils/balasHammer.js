...
export function solveBalasHammer(costs, offerOrig, demandOrig) {
  ...
  // Final step (summary)
// Préparation du breakdown Z
let zBreakdown = [];
let totalCost = 0;
for (let i = 0; i < allocations.length; i++) {
  for (let j = 0; j < allocations[0].length; j++) {
    const q = allocations[i][j];
    if (typeof q === "number" && q > 0) {
      const c = costs[i][j];
      zBreakdown.push({ i, j, cost: c, quantity: q });
      totalCost += q * c;
    }
  }
}
...
steps.push({
  allocations: allocations.map(r => [...r]),
  offer: [...offer],
  demand: [...demand],
  note: "Fin de l'algorithme — solution de base obtenue",
  isFinalBalasHammerStep: true,
  totalCost,
  zBreakdown
});
...
export function calculateTotalCost(allocations, costs) {
  ...
  for (let i = 0; i < allocations.length; i++) {
    for (let j = 0; j < allocations[0].length; j++) {
      const q = allocations[i][j];
      if (typeof q === "number" && q > 0) z += q * costs[i][j];
    }
  }
  return z;
}
